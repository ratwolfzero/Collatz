...
class CollatzShadowExplorer(QMainWindow):
    ...
    def _enforce_panel_limit(self, checked):
        ...
        if len(active_panels) > 4:
            ...
            self.summary_label.setText("⚠️ <b>Display Limit Reached:</b> Please deselect an existing panel before adding a new one. Maximum 4 panels supported concurrently.")
    ...
    def _draw_turtle_walk(self, ax, parities):
        ...
        ax.scatter([0], [0], color="#48bb78", zorder=5, label="Start") # Start point
        ax.scatter([x[-1]], [y[-1]], color="#f56565", zorder=5, label="End") # End point
    ...
    def _draw_shannon_entropy(self, ax, parities, window, block_length):
        if len(parities) < window + block_length:
            ax.text(0.5, 0.5, f"Window ({window}) too large for data length.", ha="center", va="center", color="#888888")
            return
        ...
        for i in range(len(parities) - window + 1):
            ...
            blocks = ["".join(str(b) for b in chunk[j : j + block_length]) for j in range(len(chunk) - block_length + 1)]
        ...
        ax.set_ylim(0, block_length) # Max possible entropy is log2(2^block_length) = block_length
